...
for k = 1:K                             % Loop over each species    
    for m = 1:3                         % Loop over each summary metric
       [S,Ss,Fx,Em,XY] = ...            % Run HDMR_EXT toolbox 
           HDMR_EXT(X,Y,options);       % Need to downselect Y!
    end
end
%<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>

% TO DO: try all kind of different ecologically relevant metrics 
% (from related literature). Then use HDMR_EXT to figure out which 
% parameters are most sensitive to what metric. This helps us to relate 
% metrics to parameters and see whether this makes sense in a time series

function [SA,Fx] = construct_tables(SA,nterms,p0,RMSE,alfa,f_ret,K, ...
...
%                                                                         %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This function returns a Table with a) sensitivity & b) emulator results %
%                                                                         %
%  SYNOPSIS                                                               %
%   [SA,Fx] = construct_tables(SA,nterms,p0,RMSE,alfa,f_ret,K, ...        %
%       C2,C3,n1,n2,n3,n,n_ns)                                            %
%                                                                         %
...
V_em_div_V_y = bsxfun(@rdivide,SA.V_em,SA.V_y); % (V_em./V_y OK in R2016)
...
for r = 1:n1
    [ij] = n1 + find(sum(C2 == r,2) == 1); [ijk] = n1 + n2 ...
        + find(sum(C3 == r,2) == 1);
...
for i = 1:n3
    r = i+n1+n2; f_ord(r) = 3;
    row_names{r} = strcat('x',num2str(C3(i,1)),'/x', ...
        num2str(C3(i,2)),'/x',num2str(C3(i,3)));
end
% add as last row name the sum of the previous rows
row_names{nr} = 'sum';
% now create column names
col_names = {'term','order','S_{a}','std.','S_{b}','std.','S', ...
    'std.','S_{T}','std.','V(i)/V(y)','std.',...
    '#select'};
% how many columns?
nc = numel(col_names);
...
for r = 2:nr
    SA(r,2) = { f_ord(r-1) }; data = [ Sa_m(r-1) Sa_range(r-1) ...
        Sb_m(r-1) Sb_range(r-1) S_m(r-1) S_range(r-1) ];
    SA(r,j+2) = arrayfun(@(X) sprintf('%4.3f',X),data(j), ...
        'UniformOutput',0);
end
...
for r = 2:n1+1
    ...
    SA(r,j+8) = arrayfun(@(X) sprintf('%4.3f',X),data(j), ...
        'UniformOutput',0);
end
...
for r = 2:nr
    ...
    SA(r,j+4) = arrayfun(@(X) sprintf('%4.3f',X),data(j-6), ...
        'UniformOutput',0);
end
...
if K > 1
    in_T = [ sum(Sa_m) , Sa_sum_range , sum(Sb_m) , Sb_sum_range , ...
        sum(S_m) , S_sum_range , nan , ...
        nan , sum(V_em_rat) , V_em_div_V_y_sum_range , nan ];
elseif K == 1
    in_T = [ sum(Sa_m) , nan , sum(Sb_m) , nan , sum(S_m) , nan , ...
        nan , nan , sum(V_em_rat) , nan , nan ];
end
% Find appropriate columns in last row
j = 2 + find(~isnan(in_T)); SA(nr+1,j) = arrayfun(@(X) ...
    sprintf('%4.3f',X),in_T(j-2),'UniformOutput',0);
...
% Now fill the columns 2 - 4 of B -> [ # terms, # coefs, RMSE, n_ns ]
for k = 2:K+1
    Fx(k,2:nc-2) = { nterms(1,k-1) p0(1,k-1) };
    Fx(k,nc) = { n_ns(1,k-1) };
end    

% Now change number of significant digits of RMSE column
Fx(2:K+1,nc-1) = arrayfun(@(X) sprintf('%4.3f',X),RMSE(1,1:K), ...
    'UniformOutput',0);
%Fx(2:K+1,nc) = { n_ns(1,1:K) };